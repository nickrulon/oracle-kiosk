...
# --------------------------------------------------
# Updated version: Removed settings sidebar. Changed header text to:
# "🧠 Grimey Super Intelligence — Brain Scan Interface"
...
USER_INSTRUCTION = (
    "When the user provides input in the following format, return one completed oracle output strictly following the templates.\n\n"
    "Name: {name}\n"
...
if submitted:
    if not OPENAI_API_KEY:
        ...
        with st.status("Neural scan in progress…", expanded=True) as status:
            ...
            try:
                text = generate_oracle_text(name, occupation, detail, birthday, length, OPENAI_MODEL)
